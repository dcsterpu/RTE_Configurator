...
#MS_PRIVATE_INIT = ['PRIVATE_INIT_VAR_8', 'PRIVATE_INIT_VAR_16', 'PRIVATE_INIT_VAR_32', 'PRIVATE_INIT_VAR_UNSPECIFIED']
MS_PRIVATE_INIT = []
...
#MS_PRIVATE_CLEARED = ['PRIVATE_CLEARED_VAR_8', 'PRIVATE_CLEARED_VAR_16', 'PRIVATE_CLEARED_VAR_32', 'PRIVATE_CLEARED_VAR_UNSPECIFIED']
MS_PRIVATE_CLEARED = []
...
#MS_PUBLIC_INIT = ['PUBLIC_INIT_VAR_8', 'PUBLIC_INIT_VAR_16', 'PUBLIC_INIT_VAR_32', 'PUBLIC_INIT_VAR_UNSPECIFIED']
MS_PUBLIC_INIT = []
...
#MS_PUBLIC_CLEARED = ['PUBLIC_CLEARED_VAR_8', 'PUBLIC_CLEARED_VAR_16', 'PUBLIC_CLEARED_VAR_32', 'PUBLIC_CLEARED_VAR_UNSPECIFIED']
MS_PUBLIC_CLEARED = []
...
def main():
    ...
    if args.disable_com_optimization:
        optimize = True
    mem_map = False
    if args.MemMap:
        mem_map = True
    ...
    if mem_map == False and (aswc_path != '' or acme_path != '' or rte_path != ''):
    ...
    if output_path:
        ...
        if output_log:
            ...
            swc_allocation, events, aswcs, swc_types = create_list(entry_list, entry_cfg_list, config_path, events,
                                                                   ...
                                                                   logger, debugger, rte, optimize)
            if mem_map:
                variables = parse_config_memmap(mem_config_path, logger)
                ret = memmap_creator(entry_list, swc_allocation, memory_mappings, memmap_adressing_mode_set, list_alloc, output_path, logger,variables, merged_files_path,bswc_files_list,os_list,output_src)
                ...
                if error_no != 0 :
                    ...
                    print("\nMemory mapping creation script finished with: " + str(info_no) + " infos, " + str(
                        warning_no) + " warnings, " + str(error_no) + " errors\n")
            if rte:
                create_configuration(events, aswcs, swc_types, output_path, optimize)
        else:
            if mem_map:
        ...
        if output_epc:
            ...
            if output_log:
                ...
                swc_allocation, events, aswcs, swc_types = create_list(entry_list, entry_cfg_list, config_path, events,
                                                                       ...
                                                                       logger, debugger, rte, optimize)
                if rte:
                    create_configuration(events, aswcs, swc_types, output_epc, optimize)
                if mem_map:
                    variables = parse_config_memmap(mem_config_path, logger)
                    ret = memmap_creator(entry_list, swc_allocation, memory_mappings, memmap_adressing_mode_set, list_alloc,
                                   output_log, logger,variables, merged_files_path,bswc_files_list,os_list,output_src)
                    error_no = error_no + ret[0]
                    info_no = info_no + ret[1]
                    warning_no = warning_no + ret[2]
                ...
                if mem_map:
                    ...
                    ret = memmap_creator(entry_list, swc_allocation, memory_mappings, memmap_adressing_mode_set,
                                         list_alloc, output_epc, logger,variables,merged_files_path,bswc_files_list,os_list,output_src)
                    memmap_creator(entry_list,swc_allocation, memory_mappings, memmap_adressing_mode_set, list_alloc, output_path,  logger,variables,merged_files_path,bswc_files_list,os_list,output_src)
                    error_no = error_no + ret[0]
                    info_no = info_no + ret[1]
                    warning_no = warning_no + ret[2]
            ...
            for index2 in range(len(event_list)):
                if index1 != index2:
                    if event_list[index1]['EVENT'] == event_list[index2]['EVENT']:
                        logger.warning("There are two definition of the constraints for the event " + event_list[index1]['EVENT'])
                        ...
                        if events[index_of_events]['SPECIFIC-TASK'] is None:
                            ...
                            if event_list[index2]['SPECIFIC-TASK'] != event_list[index1]['SPECIFIC-TASK'] and event_list[index1]['SPECIFIC-TASK'] is not None:
                                logger.error(
                        ...
                        if not events[index_of_events]['UNMAPPED']:
                            events[index_of_events]['UNMAPPED'] = event_list[index1]['UNMAPPED']
    return events
...
def arg_parse(parser):
    ...
    parser.add_argument('-disable_com_optimization', action="store_const", const="-disable_com_optimization", required=False, default="")
...
def create_list(file_arxml_list, files_list, config, events, aswcs, swc_types, output_path, default_duration, logger, debugger, rte, optimization):
    events_rte = []
    events_aswc = []
    swc_allocation = []
    ibehaviors = []
    ...
    for elem_rte in events_rte:
        for elem_aswc in events_aswc:
            if elem_rte['NAME'] == elem_aswc['NAME']:
                ...
                elem_aswc['UNMAPPED'] = elem_rte['UNMAPPED']
                if elem_rte['DURATION'] != '0':
                    elem_aswc['DURATION'] = elem_rte['DURATION']
                if not optimization:
                    if elem_rte['COM-ACCESS'] is False and elem_aswc['TYPE'] == "PER":
...
def create_configuration(events, aswcs, swc_types, output_path, optimize):
    ...
    output.write(output_path + '/Rte.epc', encoding='UTF-8', xml_declaration=True, method="xml",
                 doctype="<!-- XML file generated by RTE_Configurator-20 -->")
    return

def memmap_creator(entry_list, swc_allocation, mms, mams, la, output_path, l,variables,merged_files_path, bswc_files_list,os_list,output_src):
    ...
    ret = create_mapping_bswc(mms, l, bswc_files_list)
    errors = errors + ret[0]
    infos = infos + ret[1]
                ...
                for ms_inter in zip(MS_INTER_CLEARED, MS_INTER_INIT, MS_INTER_NOINIT):
                    ...
                    if len(pattern) > 0:
                        if memory_section['NAME_MS'] not in pattern:
                            memory_section['MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_CLEARED_' + \
                                                                        pattern[0][0] + '_' + pattern[0][1]
                        else:
                            c = ''
                            p = ''
                            for core in list_cores:
                            ...
                            if c != '' and p != '':
                                memory_section[
                                    'MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_CLEARED_' + p + '_' + c
                            else:
                    ...
                    if len(pattern) > 0:
                        if memory_section['NAME_MS'] not in pattern:
                            memory_section['MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_INIT_' + \
                                                                        pattern[0][0] + '_' + pattern[0][1]
                        else:
                            c = ''
                            p = ''
                            ...
                            if c != '' and p != '':
                                memory_section[
                                    'MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_INIT_' + p + '_' + c
                            else:
                                l.error("Core or partition from " + pattern + " is not present in the OsConfig file!")
                    ...
                    if len(pattern) > 0:
                        if memory_section['NAME_MS'] not in pattern:
                            memory_section['MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_NO_INIT_' + \
                                                                        pattern[0][0] + '_' + pattern[0][1]
                        else:
                            c = ''
                            p = ''
                            ...
                            if c != '' and p != '':
                                memory_section[
                                    'MEMMAP_ADDRESS_MODE_SET'] = 'MemMapAddressingModeSet_VSM_INTER_NO_INIT_' + p + '_' + c
                            else:
                                l.error("Core or partition from " + pattern + " is not present in the OsConfig file!")
